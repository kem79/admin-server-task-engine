...
# APIs in Data-Metrics-Service
    ...
    def hosts_performance_summary(self, params):
        ...
        fromTime = '1556935645705'
        toTime = '1557022045705'
        self.sendGetRequest('/api/v1/hosts/performance/summary%s&metric_type=%s&from_timestamp=%s&to_timestamp=%s'
                            % (params, 'cpu', fromTime, toTime))
        self.sendGetRequest('/api/v1/hosts/performance/summary%s&metric_type=%s&from_timestamp=%s&to_timestamp=%s'
                            % (params, 'mem', fromTime, toTime))
        self.sendGetRequest('/api/v1/hosts/performance/summary%s&metric_type=%s&from_timestamp=%s&to_timestamp=%s'
                            % (params, 'disk', fromTime, toTime))
        self.sendGetRequest('/api/v1/hosts/performance/summary%s&metric_type=%s&from_timestamp=%s&to_timestamp=%s'
                            % (params, 'net', fromTime, toTime))

    def hosts_performance_details(self, params):
        # Time range is 24 hours as UI default
        fromTime = '1557014815259'
        toTime = '1557022015261'
        self.sendGetRequest('/api/v1/hosts/performance/details%s&host_uuid=%s&from_timestamp=%s&to_timestamp=%s&metric_type=%s&%s'
                            % (params, self.hostId, fromTime, toTime, 'cpu', 'show_anomalies=true'))
        self.sendGetRequest('/api/v1/hosts/performance/details%s&host_uuid=%s&from_timestamp=%s&to_timestamp=%s&metric_type=%s&%s'
                            % (params, self.hostId, fromTime, toTime, 'mem', 'show_anomalies=true'))
        self.sendGetRequest('/api/v1/hosts/performance/details%s&host_uuid=%s&from_timestamp=%s&to_timestamp=%s&metric_type=%s&%s'
                            % (params, self.hostId, fromTime, toTime, 'disk', 'show_anomalies=true'))
        self.sendGetRequest('/api/v1/hosts/performance/details%s&host_uuid=%s&from_timestamp=%s&to_timestamp=%s&metric_type=%s&%s'
                            % (params, self.hostId, fromTime, toTime, 'net', 'show_anomalies=true'))
